...
class ProjectiveView {
  constructor(ui,
    ...
    install_edge_hanler(name, handle, lines, direction) {
      handle.onmouseenter = () => {
        if (this.isActive()) {
          ...
          if (name) { name.split(',').forEach(n => this.hightlight_line(lines[n])) }
        }
      }
    ...
    install_buttons() {
      ...
      const ignore_left_mouse_down = (event) => {
        if (event.which == 1) {
          event.stopPropagation()
        }
      }
...
class ProjectiveViewOps {
  constructor(ui, editorCfg, boxEditor, views, boxOp, func_on_box_changed, func_on_box_remove) {
    ...
    function default_on_focus() {
      ...
      if (scope.box && scope.box.boxEditor.boxEditorManager) { scope.box.boxEditor.boxEditorManager.globalHeader.update_box_info(scope.box) }
    }
    ...
    function auto_stick(delta, direction, use_box_bottom_as_limit) {
      ...
      const new_dim = scope.box.world.lidar.get_points_dimmension_of_box(virtbox, use_box_bottom_as_limit)
    ...
    function on_z_wheel(wheel_direction) {
      const multiplier = get_wheel_multiplier(wheel_direction)
      const newRatio = scope.views[0].zoom_ratio *= multiplier
    ...
    function on_y_wheel(wheel_direction) {
      const multiplier = get_wheel_multiplier(wheel_direction)
      const newRatio = scope.views[1].zoom_ratio *= multiplier
    ...
    function on_x_wheel(wheel_direction) {
      const multiplier = get_wheel_multiplier(wheel_direction)
      const newRatio = scope.views[2].zoom_ratio *= multiplier
