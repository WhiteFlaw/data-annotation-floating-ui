    ...
    install_edge_hanler(name, handle, lines, direction) {
        ...
        handle.onmousedown = (event) => {
            ...
            svg.onmouseup = (event) => {
                ...
                if (direction) {
                    this.on_edge_changed(ratio_delta, direction, event.ctrlKey, event.shiftKey); // 改变至最终结果,这是个传入的函数
                    ...
                    this.on_moved(ratio_delta); // on_x_move调用
                }
            }
            ...
            svg.onmousemove = (event) => {
                ...
                this.move_lines(handle_delta, direction); // 只是负责2D视觉
            }
        };
    }
...
class ProjectiveViewOps {
    constructor(ui, editorCfg, boxEditor, views, boxOp, func_on_box_changed, func_on_box_remove) {
        ...
        function on_edge_changed(delta, direction) {
            ...
            scope.on_box_changed(scope.box); // 更新3D
        }
        ...
        this.z_view_handle.ui.querySelector('#subview-z-up').onclick = () => {
            on_z_moved({ x: 0, y: 0.02 })
        }
        this.z_view_handle.ui.querySelector('#subview-z-down').onclick = () => {
            on_z_moved({ x: 0, y: -0.02 })
        }
        this.z_view_handle.ui.querySelector('#subview-z-left').onclick = () => {
            on_z_moved({ x: -0.02, y: 0 })
        }
        this.z_view_handle.ui.querySelector('#subview-z-right').onclick = () => {
            on_z_moved({ x: 0.02, y: 0 })
        }
        ///////////////////////////////////////////////////////////////////////////////////
        ...
        this.y_view_handle.ui.querySelector('#subview-y-up').onclick = () => {
            on_y_moved({ x: 0, y: 0.02 })
        }
        this.y_view_handle.ui.querySelector('#subview-y-down').onclick = () => {
            on_y_moved({ x: 0, y: -0.02 })
        }
        this.y_view_handle.ui.querySelector('#subview-y-left').onclick = () => {
            on_y_moved({ x: -0.02, y: 0 })
        }
        this.y_view_handle.ui.querySelector('#subview-y-right').onclick = () => {
            on_y_moved({ x: 0.02, y: 0 })
        }
        ...
        this.x_view_handle.ui.querySelector('#subview-x-up').onclick = () => {
            on_x_moved({ x: 0, y: 0.02 })
        }
        this.x_view_handle.ui.querySelector('#subview-x-down').onclick = () => {
            on_x_moved({ x: 0, y: -0.02 })
        }
        this.x_view_handle.ui.querySelector('#subview-x-left').onclick = () => {
            on_x_moved({ x: -0.02, y: 0 })
        }
        this.x_view_handle.ui.querySelector('#subview-x-right').onclick = () => {
            on_x_moved({ x: 0.02, y: 0 })
        }
