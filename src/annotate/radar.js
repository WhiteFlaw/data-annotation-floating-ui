...
function Radar(sceneMeta, world, frameInfo, radarName) {
  ...
  this.go = function(webglScene, on_go_finished) {
    ...
    if (this.preloaded) {
      if (this.elements) {
        ...
        if (!this.showPointsOnly) { this.elements.arrows.forEach(a => this.webglScene.add(a)) }
  ...
  this.unload = function(keep_box) {
    if (this.elements) {
      ...
      if (!this.showPointsOnly) { this.elements.arrows.forEach(a => this.webglScene.remove(a)) }
  ...
  this.move_points = function(points, box) {
    const trans = euler_angle_to_rotate_matrix_3by3(box.rotation)
