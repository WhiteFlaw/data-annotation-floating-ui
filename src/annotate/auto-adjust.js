...
function AutoAdjust(boxOp, mouse, header) {
  ...
  this.followStaticObjects = function(box) {
    ...
    const staticObjects = world.annotation.boxes
      ...
      .map(refObj => {
        const coord = euler_angle_to_rotate_matrix_3by3(refObj.rotation)
    ...
    worldList.forEach(w => {
      ...
      const existedBox = w.annotation.boxes.find(b => b.obj_track_id == box.obj_track_id)
      ...
      let candPoseSets = staticObjects.map(refObj => {
        const refObjInW = w.annotation.boxes.find(b => b.obj_track_id == refObj.obj_track_id)
        if (!refObjInW) {
        ...
        const coord = euler_angle_to_rotate_matrix_3by3(refObjInW.rotation)
      ...
      if (existedBox) {
        ...
        const newBox = w.annotation.add_box(newObjPos,
          box.scale,
          newObjRot,
          box.obj_type,
          box.obj_track_id,
  ...
  this.followsRef = function(box) {
    ...
    const world = box.world
    const refObj = world.annotation.boxes.find(b => b.obj_track_id == this.marked_object.ann.obj_id)
    if (refObj) {
  ...
  this.smart_paste = function(selected_box, add_box, on_box_changed) {
    ...
    if (!box) {
      const sceneP = this.mouse.get_mouse_location_in_world()
