...
class Data {
  ...
  async readSceneList() {
    // const req = new Request("/dev-ann-api/get_all_scene_desc");
    // let init = {
    //     method: 'GET',
    ...
    //         console.log(this.sceneDescList)
    //         return ret;
    //     })
    //     .catch(reject => {
    ...
    if (taskId) {
      ...
      // console.log(res)
      if (res.success) {
        this.sceneDescList = res.data.scene
        this.sceneAllData = res.data
        // console.log(8888888, this.sceneDescList,this.sceneAllData)
      } else {
  ...
  changeAnno(oldFrame, newWorldData) {
    // webglGroup会随着_preload自动更新, 数据正确就不用担心
    let oldWorld = this.worldList.find((w) => w.frameInfo.frame === oldFrame)
  ...
  getWorldByFrame(frame) {
    // search world in worldList by frame.
    let world = this.worldList.find((w) => {
  ...
  readSceneMetaData(sceneName) {
    this.meta[sceneName] = {...this.sceneAllData}
    return {...this.sceneAllData}
    ...
    //             console.log('点击之后',this,sceneMeta)
    //             resolve(sceneMeta);
    //         }
...
const getSceneMateByTaskId = (taskId) => {
  return GET(`/admin/tagging/scene-mate/${taskId}`)
}
