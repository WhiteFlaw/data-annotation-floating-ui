...
function FrameInfo(data, sceneMeta, sceneName, frame) {
  // console.log('是否有图片数据', data, sceneMeta, sceneName, frame)
  ...
  this.annotation_format = this.sceneMeta.box_type //xyz(24 number), csr(center, scale, rotation, 9 number)
  // 取到所有图片文件地址数据
  this.picPathArr = this.data.sceneAllData.homework_list
  this.workPath = this.picPathArr.find((f) => f.name === frame)['workPath']
  ...
  this.get_pcd_path = function () {
    ...
    return `${process.env.VUE_APP_TOOL_STATIC_URL}/${this.workPath}`
  }
...
function Images(sceneMeta, sceneName, frame) {
  ...
  // 获取图片地址
  this.cameraFrontPath = sceneMeta.homework_list.find((f) => f.name === frame)['cameraFrontPath']
...
function World(data, sceneName, frame, coordinatesOffset, on_preload_finished) {
  ...
  this.calcTransformMatrix = function () {
    if (this.egoPose.egoPose) {
      ...
      let trans_lidar_ego = new THREE.Matrix4().makeRotationFromEuler(new THREE.Euler(0, 0, Math.PI, 'ZYX')).setPosition(0, 0, 0.4)
      ...
      if (this.data.cfg.coordinateSystem == 'utm') this.trans_lidar_scene = new THREE.Matrix4().multiplyMatrices(trans_utm_scene, this.trans_lidar_utm)
