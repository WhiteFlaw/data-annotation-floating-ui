...
function FrameInfo(data, sceneMeta, sceneName, frame) {
  ...
  this.get_pcd_path = function() {
    return `${this.prefixes}data/${this.scene}/lidar/${this.frame}${this.sceneMeta.lidar_ext}`
  }
  this.get_radar_path = function(name) {
    return `${this.prefixes}data/${this.scene}/radar/${name}/${this.frame}${this.sceneMeta.radar_ext}`
  }
  this.get_aux_lidar_path = function(name) {
    return `${this.prefixes}data/${this.scene}/aux_lidar/${name}/${this.frame}${this.sceneMeta.radar_ext}`
  }

  this.get_anno_path = function() {
    if (this.annotation_format == 'psr') {
      return `${this.prefixes}data/${this.scene}/label/${this.frame}.json`
    } else {
      return `${this.prefixes}data/${this.scene}/bbox.xyz/${this.frame}.bbox.txt`
    }
  }
  ...
  this.xyz_to_psr = function(ann_input) {
    var ann = []
    if (ann_input.length == 24) { ann = ann_input } else {
...
function World(data, sceneName, frame, coordinatesOffset, on_preload_finished) { // world初始实例化不包括webglGroup, 所以会直接执行preload以生成webglGroup
  ...
  this.calcTransformMatrix = function() {
    if (this.egoPose.egoPose) {
      ...
      const trans_lidar_ego = new THREE.Matrix4().makeRotationFromEuler(new THREE.Euler(0, 0, Math.PI, 'ZYX'))
        .setPosition(0, 0, 0.4)

      // let trans_ego_utm = euler_angle_to_rotate_matrix(thisRot, posDelta, "ZXY");
      const trans_ego_utm = new THREE.Matrix4().makeRotationFromEuler(new THREE.Euler(thisRot.x, thisRot.y, thisRot.z, 'ZXY'))
        .setPosition(posDelta.x, posDelta.y, posDelta.z)

      const trans_utm_scene = new THREE.Matrix4().identity().setPosition(this.coordinatesOffset[0], this.coordinatesOffset[1], this.coordinatesOffset[2])
      ...
      if (this.data.cfg.coordinateSystem == 'utm') { this.trans_lidar_scene = new THREE.Matrix4().multiplyMatrices(trans_utm_scene, this.trans_lidar_utm) } else { this.trans_lidar_scene = trans_utm_scene } // only offset.
  ...
  this.lidarPosToScene = function(pos) {
    const tp = new THREE.Vector3(pos.x, pos.y, pos.z).applyMatrix4(this.trans_lidar_scene)
  ...
  this.lidarPosToUtm = function(pos) {
    const tp = new THREE.Vector3(pos.x, pos.y, pos.z).applyMatrix4(this.trans_lidar_utm)
  ...
  this.lidarRotToScene = function(rotEuler) {
    ...
    const localToGlobalRot = new THREE.Quaternion().setFromRotationMatrix(this.trans_lidar_scene)
  ...
  this.lidarRotToUtm = function(rotEuler) {
    ...
    const rotL = new THREE.Quaternion().setFromEuler(rotEuler)
    const localToGlobalRot = new THREE.Quaternion().setFromRotationMatrix(this.trans_lidar_utm)
  ...
  this.utmRotToLidar = function(rotEuler) {
    ...
    const trans = new THREE.Quaternion().setFromRotationMatrix(this.trans_utm_lidar)
  ...
  this.preload = function(on_preload_finished) { // on_preload_finished没传
    ...
    const _preload_cb = () => this.on_subitem_preload_finished(on_preload_finished)
