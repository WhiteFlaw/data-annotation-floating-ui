...
function Annotation(sceneMeta, world, frameInfo) {
  ...
  this.vector_global_to_ann = function (v) {
    let posG = new THREE.Vector3(v[0] + this.world.coordinatesOffset[0], v[1] + this.world.coordinatesOffset[1], v[2] + this.world.coordinatesOffset[2])
  ...
  this.if_draw_box = (boxes_draw_no) => {
    ...
    for (let i = 0; i < boxes.length; i++) {
      boxes[i]['draw'] = true
      for (let j = 0; j < boxes_draw_no.length; j++) {
        // 需要设置fraw = false的放这, 但是也在这里面设置true就设置不上了, 应该放这个循环外
  ...
  this.load_annotation = async function (on_load) {
    const that = this
    if (this.data.cfg.disableLabels) {
      on_load([])
    } else {
      const homeworkId = this.world?.sceneMeta?.homework_list.find((h) => h.name === this.world.frameInfo.frame)['id']
      const res = await getAnnotationData(homeworkId)
      if (res.success && res.data ) {
        let ann = this.frameInfo.anno_to_boxes(res.data.dataJson || '[]')
      ...
      //     if (this.status == 200) {
      //       console.log(777777777777,this.responseText)
  ...
  const getAnnotationData = (homeworkId) => {
    return GET(`/admin/label/${homeworkId}`)
  }
  ...
  this.createOneBoxByAnn = function (annotation) {
    ...
    let mesh = this.createCuboid(b.psr.position, b.psr.scale, b.psr.rotation, b.obj_type, b.obj_id, b.obj_trunk, b.obj_occlu)
