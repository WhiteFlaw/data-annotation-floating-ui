...
function Annotation(sceneMeta, world, frameInfo) {
  ...
  this.ann_to_vector_global = function(box) {
    const posG = this.world.lidarPosToScene(box.position)
    const rotG = this.world.lidarRotToScene(box.rotation)
  ...
  this.vector_global_to_ann = function(v) {
    const posG = new THREE.Vector3(v[0] + this.world.coordinatesOffset[0],
      v[1] + this.world.coordinatesOffset[1],
      v[2] + this.world.coordinatesOffset[2])
    const rotG = new THREE.Euler(v[3], v[4], v[5])

    const rotL = this.world.sceneRotToLidar(rotG)
    const posL = this.world.scenePosToLidar(posG)
  ...
  this.add_box = function(pos, scale, rotation, obj_type, track_id, obj_attr) {
    ...
    const mesh = this.createCuboid(pos, scale, rotation, obj_type, track_id, objAttr)
  ...
  this.load_annotation = function(on_load) {
    if (this.data.cfg.disableLabels) {
      ...
      xhr.onreadystatechange = function() {
        ...
        if (this.status == 200) {
          const ann = _self.frameInfo.anno_to_boxes(this.responseText)
          on_load(ann)
        }
