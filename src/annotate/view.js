...
function BoxView(ui, mainViewContainer, scene, renderer, viewManager) {
    ...
    this.updateTitleInfo = function (box) {
        const scale = box.scale;
        this.ui.querySelector('#sub-views-size-top-val').innerHTML = `宽: ${scale.y.toFixed(2)} 长: ${scale.x.toFixed(2)} | 点数: ${box.world && box.world.lidar.get_box_points_number(box)}`;
        this.ui.querySelector('#sub-views-size-left-val').innerHTML = `长: ${scale.x.toFixed(2)} 高: ${scale.z.toFixed(2)}`;
        this.ui.querySelector('#sub-views-size-front-val').innerHTML = `宽: ${scale.y.toFixed(2)} 高: ${scale.z.toFixed(2)}`;
    };
