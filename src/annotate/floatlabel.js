...
class FastToolBox {
  constructor(ui, eventHandler) {
    ...
    this.ui.querySelector('#label-more').onmouseenter = function(event) {
      ...
      const ui = event.target.querySelector('#object-dropdown-menu')
  ...
  show() { // it be called when floating-things needs to show.
    if (document.querySelector('#if-default-use').checked === true) {
      const default_category = document.querySelector('#category-selector').value
      const default_attribute = document.querySelector('#attribute-selector').value
...
class FloatLabelManager {
    ...
    coord_to_pixel(p) {
      var width = this.container.clientWidth; var height = this.container.clientHeight
      var widthHalf = width / 2; var heightHalf = height / 2
