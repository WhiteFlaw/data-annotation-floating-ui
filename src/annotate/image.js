...
import { imageWrapperTemplate } from './template/imageWrapperTemplate'
import {
  vector4to3,
  vector3_nomalize,
  psr_to_xyz, matmul,
...
function BoxImageContext(ui) {
  ...
  this.updateFocusedImageContext = function(box) {
    ...
    const bestImage = choose_best_camera_for_point(
      scene_meta,
      box.position)
  ...
  function crop_image(imgWidth, imgHeight, clientWidth, clientHeight, corners) {
    var maxx = 0; var maxy = 0; var minx = imgWidth; var miny = imgHeight
...
class ImageContext extends MovableView {
  constructor(parentUi, name, autoSwitch, cfg, on_img_click) {
    ...
    const handle = ui.querySelector('#move-handle')
    super(handle, ui)
  ...
  setImageName(name) {
    ...
    point_color_by_distance(x, y) {
      ...
      const p = this.img_lidar_point_map[y * this.img.width + x]

      let distance = Math.sqrt(p[1] * p[1] + p[2] * p[2] + p[3] * p[3])

      if (distance > 60.0) { distance = 60.0 } else if (distance < 10.0) { distance = 10.0 }
    ...
    points_to_svg(points, trans_ratio, cssclass, radius = 2) {
      ...
      for (let i = 0; i < ptsFinal.length; i += 2) {
        ...
        if (!cssclass) {
          const image_x = points[i]
          const image_y = points[i + 1]
    ...
    draw_point(x, y) {
      ...
      const pts_svg = this.points_to_svg([x, y], trans_ratio, 'radar-points')
    ...
    draw_svg() {
      ...
      if (this.cfg.projectRadarToImage) {
        this.world.radars.radarList.forEach(radar => {
          ...
          if (ptsOnImg && ptsOnImg.length > 0) {
            const pts_svg = this.points_to_svg(ptsOnImg, trans_ratio, radar.cssStyleSelector)
            svg.appendChild(pts_svg)
          }
        })
      }
      ...
      if (this.cfg.projectLidarToImage) {
        const pts = this.world.lidar.get_all_points()
        const ptsOnImg = points3d_to_image2d(pts, calib, true, this.img_lidar_point_map, img.width, img.height)

        // there may be none after projecting
        if (ptsOnImg && ptsOnImg.length > 0) {
          const pts_svg = this.points_to_svg(ptsOnImg, trans_ratio)
          svg.appendChild(pts_svg)
        }
      }
    }
...
class ImageContextManager {
  constructor(parentUi, selectorUi, cfg, on_img_click) {
    ...
    this.selectorUi.querySelector('#camera-list').onclick = (event) => {
      const cameraName = event.target.innerText
  ...
  setDefaultBestCamera(c) {
    ...
    addImage(name, autoSwitch) {
      ...
      const image = new ImageContext(this.parentUi, name, autoSwitch, this.cfg, this.on_img_click)
      ...
      const ui = this.selectorUi.querySelector('#camera-item-' + selectorName)
