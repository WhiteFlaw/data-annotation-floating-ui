...
function Editor(editorUi, wrapperUi, editorCfg, data, name = 'editor') {
  ...
  this.init = function (editorUi) {
    ...
    this.header = new Header(
      ...
      () => {
        this.use_attribute_changed()
      }, // enable the default attribute.
      () => {
        this.use_category_changed()
      }, // enable the default category.
      () => {
        this.use_previous_frame_click()
      }, // use previous frame.
      (isShow) => {
        this.onFrontCameraClicked(isShow)
      }, // show/hide front camera.
      (isShow) => {
        this.onLeftCameraClicked(isShow)
      }, // show/hide left camera.
      (isShow) => {
        this.onRightCameraClicked(isShow)
      }, // show/hide right camera.
      (isShow) => {
        this.onToggleImageEditor(isShow)
      },
      () => {
        this.undo_click()
      },
    ...
    this.commentManager = new CommentManager(this.editorUi.querySelector('#content'), this.data)
    //
    ...
    this.imageViewer = new ImageViewer(this.editorUi.querySelector('#content'), this.editorCfg)

    if (!this.editorCfg.disableRangeCircle) this.addRangeCircle()

    this.floatLabelManager = new FloatLabelManager(this.editorUi, this.container, this.viewManager.mainView, function (box) {
      self.selectBox(box)
    })
    ...
    this.objectTrackView = new Trajectory(this.editorUi.querySelector('#object-track-wrapper'))

    this.infoBox = new InfoBox(this.editorUi.querySelector('#info-wrapper'))

    this.cropScene = new CropScene(this.editorUi.querySelector('#crop-scene-wrapper'), this)
    ...
    this.boxEditor.moveHandle = new MovableView(boxEditorUi.querySelector('#focuscanvas'), boxEditorUi.querySelector('#sub-views'), () => {
  ...
  this.addRangeCircle = function () {
    ...
    var box = new THREE.LineSegments(bbox, new THREE.LineBasicMaterial({ color: 0x888800, linewidth: 1, opacity: 0.5, transparent: true }))
    ...
    switch (event.currentTarget.id) {
      ...
      case 'label-gen-id':
        ...
        self.fastToolBox.setValue(
          ...
          self.selected_box.world.lidar.get_box_points_number(self.selected_box)
        )
      ...
      case 'label-paste':
        ...
        this.boxOp.auto_rotate_xyz(this.selected_box, null, null, (b) => this.on_box_changed(b), 'noscaling')
        //event.currentTarget.blur();
        break
      ...
      case 'label-batchedit':
        {
          ...
          this.editBatch(this.data.world.frameInfo.scene, this.data.world.frameInfo.frame, this.selected_box.obj_track_id, this.selected_box.obj_type)
        }
        break
  ...
  this.showTrajectory = function () {
    ...
    this.objectTrackView.setObject(this.selected_box.obj_type, this.selected_box.obj_track_id, tracks, (targetFrame) => {
      //onExit
      ...
      case 'cm-paste':
        {
          ...
          if (!event.shiftKey) {
            ...
            this.boxOp.auto_rotate_xyz(box, null, null, (b) => this.on_box_changed(b), 'noscaling')
          }
        }
        break
      case 'cm-prev-frame':
      ...
      case 'cm-reload-all':
        {
          ...
          if (modifiedFrames.length > 0) {
            this.infoBox.show('Confirm', `Discard changes to ${modifiedFrames.length} frames, continue to reload?`, ['yes', 'no'], (choice) => {
              if (choice == 'yes') {
  ...
  this.editBatch = function (sceneName, frame, objectTrackId, objectType) {
    ...
    this.boxEditorManager.edit(this.data, this.data.getMetaBySceneName(sceneName), frame, objectTrackId, objectType, (targetFrame, targetTrackId) => {
      //on exit
  ...
  this.create_box_by_points = function (points, rotationZ) {
    ...
    let transToBoxMatrix = new THREE.Matrix4().makeRotationFromEuler(localRot).setPosition(0, 0, 0).invert()
  ...
  this.selectBox = function (object) {
    if (this.selected_box != object) {
      ...
      if (!this.editorCfg.disableMainImageContext) {
        var best_camera = this.imageContextManager.choose_best_camera_for_point(this.selected_box.world.frameInfo.sceneMeta, this.selected_box.position)
  ...
  this.on_box_changed = function (box) {
    if (!this.imageContextManager.hidden()) this.imageContextManager.boxes_manager.update_box(box)
  ...
  this.add_global_obj_type = function () {
    ...
    for (var o in obj_type_map) {
      var rule =
        '.' + o + '{color:' + obj_type_map[o].color + ';' + 'stroke:' + obj_type_map[o].color + ';' + 'fill:' + obj_type_map[o].color + '22' + ';' + '}'
    ...
    for (let idx = 0; idx <= 32; idx++) {
      ...
      var rule = '.color-' + idx + '{color:' + color + ';' + 'stroke:' + color + ';' + 'fill:' + color + '22' + ';' + '}'
      sheet.insertRule(rule, sheet.cssRules.length)
    }
