...
var Header = function(ui, data, cfg, onSceneChanged, onFrameChanged, onObjectSelected, onCameraChanged, onIfDefaultUseChanged, onUsePreviousFrameClicked) {
  ...
  this.updateSceneList = function(sceneDescList) {
    ...
    for (const scene in sceneDescList) {
      if (data.sceneDescList[scene]) { scene_selector_str += '<option value=' + scene + '>' + scene + ' - ' + data.sceneDescList[scene].scene + '</option>' } else { scene_selector_str += '<option value=' + scene + '>' + scene + '</option>' }
    }
